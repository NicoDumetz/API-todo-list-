...
exports.pick_todo = function(user_id, callback) {
    db.execute('SELECT id, title, description, created_at, due_time, user_id, status FROM todo WHERE id = ?', [user_id], (err, results) => {
        if (err)
        ...
        if (results.length > 0) {
            ...
            const form = new Date(todo_info.created_at).toISOString().slice(0, 19).replace('T', ' ');
            todo_info.created_at = form;
    ...
    db.execute( 'INSERT INTO todo (title, description, due_time, user_id, status) VALUES (?, ?, ?, ?, ?)',
        ...
        (err, result) => {
            ...
            if (result.affectedRows > 0) {
                exports.pick_todo(result.insertId, (err, todo_info) => {
                    if (err)
                        return callback(err);
